...
| Лучший offline       | `Val RMSLE = 0.3730` на фичах с лагами по факту (на holdout это оптимистично) |
| Реалистичный offline | `~0.397` в рекурсивной валидации                                              |
...
## Почему этот проект выглядит сильным

- Пайплайн от сырых CSV до API и Docker.
- Один и тот же код фичей используется в ноутбуке и сервисе.
- Есть не только подбор гиперпараметров, но и проверяемые гипотезы по данным.
- Есть честный разбор ошибки, из-за которой на реальном инференсе проседало качество.
- Есть объяснимость прогноза через `pred_contrib` LightGBM в `/predict/explain`.
